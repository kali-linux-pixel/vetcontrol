...
export default function SettingsPage() {
  ...
  const [clinicName, setClinicName] = useState('VetControl Downtown');
  const [clinicEmail, setClinicEmail] = useState('contact@vetcontrol.com');
  const [clinicPhone, setClinicPhone] = useState('(555) 010-8900');
  
  const [vetList, setVetList] = useState([
    { name: 'Dr. Elizabeth Blackwell', specialty: 'General Practice, Feline Medicine', license: 'DVM-90184' },
        ...
        <Card className="border border-neutral-100 bg-white shadow-xs">
          ...
          <CardContent className="p-6 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-neutral-500">Operational Address</label>
                <Input 
                  defaultValue="124 Grand Ave, Suite 3, New York, NY" 
                  className="h-9 text-xs" 
                />
              </div>
            </div>
          ...
          <CardContent className="p-6 space-y-4">
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-semibold text-neutral-800">Weekly Revenue Summaries</h4>
                  <p className="text-[10px] text-neutral-400 mt-0.5">Send a financial performance CSV statement to clinic administrators every Sunday.</p>
                </div>
                <button 
                  onClick={() => handleToggle('autoReports')}
                  className={cn(
                    "relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-hidden",
                    toggles.autoReports ? "bg-emerald-600" : "bg-neutral-200"
                  )}
                >
                  <span className={cn(
                    "pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out",
                    toggles.autoReports ? "translate-x-4" : "translate-x-0"
                  )} />
                </button>
              </div>
            </div>
